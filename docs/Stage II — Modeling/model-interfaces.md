...
- [x] Checkpoint/version naming conventions (scene adapter loads `models/scene/best_model.pkl`; EmoNet adapter expects `models/emonet/pretrained/emonet_*.pth`).

## Scene Adapter: SceneCLIPAdapter
- Location: `models/scene/clip_vit_scene_adapter.py`
- Constructor highlights:
  - `model_name="openai/clip-vit-base-patch32"`
  - `tta` controls the default number of MC samples; `auto_load_best=True` loads `best_model.pkl`
