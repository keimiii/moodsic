...
- Applies to one-shot video evaluation (e.g., `notebooks/Inference/e2e_video_to_fusion.ipynb`).
...
- Demo backend: `backend/helpers/process_video.py` reads the VEATIC parquet exports (`results/inference/pipeline_results_*.parquet`), enriches them with song metadata, and returns fused means, MAE, comments, and cluster info to the React frontend. That path assumes the parquet and DEAM artifacts are already generated by the notebooks/scripts listed above.

## References

- Notebook: `notebooks/Inference/e2e_video_to_fusion.ipynb`
- Batch export: `notebooks/Inference/batch_video_fusion_export.ipynb`
...
- Backend enrichment: `backend/helpers/process_video.py`, `backend/helpers/song_recommendation.py`
